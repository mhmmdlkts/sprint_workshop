...
public class InputInterface : MonoBehaviour {
	...
	public static Vector3 GetReverseProjection(GameObject gameObject, Vector3 position)
	{
		Vector3 _screenObj = Camera.main.WorldToScreenPoint(gameObject.transform.position);
		Vector3 _screenMouse = new Vector3(position.x, position.y, _screenObj.z);
		return Camera.main.ScreenToWorldPoint(_screenMouse);
	}

	public static Vector3 GetReverseProjection(GameObject gameObject)
	{
		return GetReverseProjection(gameObject, position);
	}
}
